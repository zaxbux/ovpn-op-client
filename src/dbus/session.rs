...
use super::constants::LogGroup;
use super::constants::LogLevel;
use crate::dbus::constants::{ClientAttentionGroup, ClientAttentionType, StatusMajor, StatusMinor};
...
impl TryFrom<(u32, u32, String)> for Status {
    ...
    fn try_from(v: (u32, u32, String)) -> std::result::Result<Self, Self::Error> {
        Ok(Self {
            major: unsafe { std::mem::transmute(<u32>::try_from(v.0)?) },
            minor: unsafe { std::mem::transmute(<u32>::try_from(v.1)?) },
            status_message: v.2,
        })
    }
}
...
impl fmt::Display for Status {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            f,
            "[{}] {} ({})",
            self.major, self.minor, self.status_message
        )
    }
}
